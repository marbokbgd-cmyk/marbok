...
export async function getPages() {
    if (typeof window !== "undefined" && IS_COMMERCIAL_SITE) return browserCatalog("pages");
...
export async function getImages() {
    if (typeof window !== "undefined" && IS_COMMERCIAL_SITE) return browserCatalog("images");
    return serverClient().fetch(groq`*[_type == "heroImages"]`);
}

export async function getHeading() {
    if (typeof window !== "undefined" && IS_COMMERCIAL_SITE) return browserCatalog("heading");
    return serverClient().fetch(groq`*[_type == "mainHeading"]`);
}

export async function getBrandImages() {
    if (typeof window !== "undefined" && IS_COMMERCIAL_SITE) return browserCatalog("brands");
    return serverClient().fetch(groq`*[_type == "brandImages"]`);
}

export async function getAboutUs() {
    if (typeof window !== "undefined" && IS_COMMERCIAL_SITE) return browserCatalog("about");
...
export async function getStores() {
    if (typeof window !== "undefined" && IS_COMMERCIAL_SITE) return browserCatalog("stores");
    return serverClient().fetch(
        groq`*[_type == "store"]{
            name,
            pib,
...
function serverClient() {
    if (!IS_COMMERCIAL_SITE) return createClient(clientConfig);
